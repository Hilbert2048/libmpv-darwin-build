...
let
  ...
  xctoolchainXcrun = pkgs.callPackage ../../utils/xctoolchain/xcrun.nix { };
  xctoolchainSwiftc = pkgs.callPackage ../../utils/xctoolchain/swiftc.nix { };
  ...
  patchedSource = pkgs.runCommand "${pname}-patched-source-${variant}-${version}" { } ''
    ...
    # Dart code will call these symbols directly
...
EOF
    ...
    patch -p1 < ${../../../patches/mpv-fix-missing-objc.patch} || echo "WARN: mpv-fix-missing-objc.patch failed, may not be needed for 0.41.0"
    patch -p1 < ${../../../patches/mpv-mix-with-others.patch} || echo "WARN: mpv-mix-with-others.patch failed, skipping"
    # Fix view.swift MetalLayer conditional compilation for non-vulkan builds
    patch -p1 < ${../../../patches/mpv-fix-view-metal-layer.patch} || echo "WARN: mpv-fix-view-metal-layer.patch failed, skipping"
...
pkgs.stdenvNoCC.mkDerivation {
  ...
  configurePhase = ''
    # Updated meson options for mpv 0.41.0
    DISABLE_ALL_OPTIONS=(
      `# booleans`
      ...
      -Djpeg=disabled `# libjpeg support`
      ...
      -Daudiotrack=disabled `# Android AudioTrack`
      -Daaudio=disabled `# Android AAudio`
      ...
      -Dswift-build=auto `# macOS Swift build tools - auto detect`
    ...
    MACOS_OPTIONS=(
      ...
      `# Swift features for macOS`
      -Dswift-build=enabled `# Enable Swift build for Cocoa integration`
    )
